...
export default async function handler(req, res) {
    ...
    try {
        ...
        const db = await client.db("hg_produtos");
        ...
        if (req.method === 'GET') {
            ...
            const { name, description, category, price, link } = req.body;

            if (!name || !description || !category || !price || !link) {
                return res.status(400).json({ error: "Todos os campos são obrigatórios." })
            }

            const novoItem = { name, description, category, price, link };
